...
# ==================================================
# GET LATEST TOTAL BUDGET (AUTO-DETECT)
# ==================================================
...
def fetch_context(program: str, year: int):
    ...
    for col_name in budget_cols:
        ...
        for r in col.find({}):
            ...
            match_text = " ".join(str(r.get(f, "")) for f in check_fields)
...
def predict_budget(req: BudgetRequest):
    ...
    for program in programs:
        ...
        for col, enc in encoders.items():
            ...
            df[col] = df[col].apply(
                lambda v: v if v in enc.classes_ else enc.classes_[0]
            )
    ...
    scale = official_total / raw_total if raw_total > 0 else 1
